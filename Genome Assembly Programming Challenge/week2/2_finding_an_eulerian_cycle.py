...
class EulerianCycle:
    def __init__(self):
        self.num_explored_edges = 0
        self.nodesWUE = dict()  # node with unused edges; value: the position of such node in the current path
        self.path = []
        is_balance = self._input()
        if not is_balance:
            print('0')
        else:
            print('1')
    ...
    def _input(self):
        ...
        self.n, self.num_explored_edges = int(data[0]), int(data[1])
        self.adj = [[] for _ in range(self.n)]
        self.unusedEdges = [[] for _ in range(self.n)]
        self.outDeg = [0] * self.n
        self.inDeg = [0] * self.n
        self.adjCurPos = [0] * self.n
        for i in range(self.num_explored_edges):
    ...
    def calculateEulerianCycle(self):
        self.explore(1)
        while self.num_explored_edges > 0:
